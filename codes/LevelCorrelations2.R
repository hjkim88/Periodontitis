...
levelCor2 <- function(epicLevelPath="//isilon.c2b2.columbia.edu/ifs/archive/shares/bisr/Papapanou/Sept_2018/PreprocessedData/Methylation/m_values_gene.txt",
  ...
  ### NEW
  png(paste0(outputDir, "DEMG_Level_Correlations_Periodontitis_Healthy3.png"), width = 2000, height = 1200, res = 150)
  ...
  for(gene in gList) {
    if(gene == "CECR1") {
      plot(as.numeric(rnaseqLev_periodontitis_healthy[gene,]), as.numeric(epic_periodontitis_healthy[gene,]),
           ...
           xlab = paste("Normalized Gene Expression"),
           ylab = paste("Normalized Methylation (M)"))
    } else {
      plot(as.numeric(rnaseqLev_periodontitis_healthy[gene,]), as.numeric(epic_periodontitis_healthy[gene,]),
           pch = 19, col = colors,
           main = paste0(gene, "\n", sprintf("r = %s, p-value = %s",
  ...
  png(paste0(outputDir, "DEMG_Level_Correlations_Periodontitis_Healthy4.png"), width = 2000, height = 1200, res = 150)
  par(mfrow=c(3,4), oma = c(0,0,3,0))
  ### for each gene draw a plot
